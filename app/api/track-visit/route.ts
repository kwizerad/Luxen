...
import {
  ...
  parseDeviceDetails,
  resolveDeviceType,
} from "@/lib/tracking/server-tracker";

function parseBrowserInfo(userAgent: string, chPlatformVersion?: string | null, chModel?: string | null) {
  ...
  if (/Android/i.test(userAgent)) {
    osName = "Android";
    osVersion = parseAndroidVersion(userAgent, chPlatformVersion);
  } else if (/iPhone|iPad|iPod/i.test(userAgent)) {
    osName = "iOS";
    const m = /OS (\d+[._]\d+(?:[._]\d+)?)/i.exec(userAgent);
    if (m?.[1]) osVersion = m[1].replace(/_/g, ".");
  } else if (/Macintosh|Mac OS X/i.test(userAgent)) {
    osName = "macOS";
    const m = /Mac OS X (\d+(?:[._]\d+)+)/i.exec(userAgent);
    if (m?.[1]) osVersion = m[1].replace(/_/g, ".");
  } else if (/Windows NT/i.test(userAgent)) {
    osName = "Windows";
    const m = /Windows NT (\d+\.?\d*)/i.exec(userAgent);
    if (m?.[1] === "10.0") osVersion = "10/11";
    else if (m?.[1]) osVersion = m[1];
