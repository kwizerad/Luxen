          ...
          <>
            ...
            <div className="sticky top-[48px] z-30 flex flex-wrap items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 min-h-[44px] h-auto border-b border-[var(--admin-border)] bg-[var(--admin-card)]">
              ...
              <div className="flex items-center flex-wrap gap-1.5 flex-shrink-0">
                ...
                {/* Auto-save indicator */}
                {isAutoSaving && (
                  <span className="inline-flex items-center gap-1 text-[10px] text-[var(--admin-muted)] opacity-60">
                    <Loader2 className="h-2.5 w-2.5 animate-spin" />
                    {t("autoSaving") || "Saving..."}
                  </span>
                )}
                {/* Save button */}
                <Button type="button" onClick={handleSave} disabled={!hasUnsavedChanges || isSaving} className="admin-btn-primary shrink-0 h-8 text-xs px-3 py-1.5 gap-1.5">
                  {isSaving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}{saveButtonLabel()}
                </Button>
              </div>
            </div>
                  ...
                  previewMode ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <LessonEditor
                        ...
                        onChange={(lesson) => actions.updateLesson(lesson.id, lesson)}
