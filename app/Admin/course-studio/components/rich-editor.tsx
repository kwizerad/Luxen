...
export function RichEditor({ content, onChange, placeholder, onEditorReady, stickyToolbar = false }: RichEditorProps) {
  ...
  const updateStats = useCallback((ed: import("@tiptap/core").Editor) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      attributes: {
        class:
          "prose max-w-none mx-0 min-h-[220px] outline-none p-3.5 sm:p-4 border border-[var(--admin-border)] bg-[var(--admin-card)] rounded-b focus-within:border-[var(--admin-primary)] focus-within:ring-1 focus-within:ring-[var(--admin-primary)]/30 transition-all",
      },
    },
  });
  ...
  useEffect(() => {
    if (!editor) return;
    // Skip if this change was just emitted by this editor instance
    if (content === lastEmittedContentRef.current) return;

    const currentJSON = editor.getJSON();
    const nextJSON = safeJSON(content);
  ...
  // Quick callout inserters
  const insertCallout = (type: "tip" | "warning" | "info" | "takeaway" | "rule") => {
    if (!editor) return;
    let html = "";
    if (type === "tip") {
  ...
  const containerClasses = isFullscreen
    ? "fixed inset-0 z-50 bg-[var(--admin-card)] p-4 sm:p-8 flex flex-col overflow-y-auto"
    : "relative";

  return (
    <div className={containerClasses}>
      {/* Top Toolbar */}
      <div className={`rich-editor-toolbar relative z-10 flex flex-wrap items-center gap-1 p-2 mb-2 min-h-[44px] h-auto border border-[var(--admin-border)] bg-[var(--admin-card)] rounded-t ${stickyToolbar && !isFullscreen ? "sticky top-[88px] z-20" : ""}`}>
        <ToolbarButton editor={editor} command="toggleBold" icon={<Bold className="h-4 w-4" />} active="bold" title="Bold (Ctrl+B)" />
        <ToolbarButton editor={editor} command="toggleItalic" icon={<Italic className="h-4 w-4" />} active="italic" title="Italic (Ctrl+I)" />
        <ToolbarButton editor={editor} command="toggleUnderline" icon={<UnderlineIcon className="h-4 w-4" />} active="underline" title="Underline (Ctrl+U)" />
        ...
        <div className="relative">
          <Button type="button" size="icon" variant="ghost" title="Text Color" onClick={() => { setShowTextColor(!showTextColor); setShowHighlight(false); setShowFontSize(false); setShowFontFamily(false); setShowCallouts(false); }} className="h-8 w-8 text-[var(--admin-text)]">
        ...
        <div className="relative">
          <Button type="button" size="icon" variant="ghost" title="Highlight Color" onClick={() => { setShowHighlight(!showHighlight); setShowTextColor(false); setShowFontSize(false); setShowFontFamily(false); setShowCallouts(false); }} className="h-8 w-8 text-[var(--admin-text)]">
        ...
        {/* Font Size */}
        <div className="relative">
          <Button type="button" size="icon" variant="ghost" title="Font Size" onClick={() => { setShowFontSize(!showFontSize); setShowTextColor(false); setShowHighlight(false); setShowFontFamily(false); setShowCallouts(false); }} className="h-8 w-8 text-[var(--admin-text)]">
        ...
        {/* Headings */}
        <ToolbarButton editor={editor} command={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} icon={<Heading1 className="h-4 w-4" />} active={{ heading: { level: 1 } }} title="Heading 1" />
        <ToolbarButton editor={editor} command={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} icon={<Heading2 className="h-4 w-4" />} active={{ heading: { level: 2 } }} title="Heading 2" />
        <ToolbarButton editor={editor} command={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} icon={<Heading3 className="h-4 w-4" />} active={{ heading: { level: 3 } }} title="Heading 3" />
        ...
        <div className="relative">
          <Button
            ...
            variant="ghost"
            title="Callout Box"
            onClick={() => { setShowCallouts(!showCallouts); setShowTextColor(false); setShowHighlight(false); setShowFontSize(false); }}
            className="h-8 px-2 text-[var(--admin-text)] text-xs gap-1"
          >
            <Sparkles className="h-3.5 w-3.5 text-amber-500" />
            <span className="text-[10px]">▾</span>
          </Button>
          {showCallouts && (
            <div className="absolute z-30 top-9 left-0 p-1.5 rounded-lg border border-[var(--admin-border)] bg-[var(--admin-card)] shadow-lg min-w-[170px] space-y-1">
              <button
                type="button"
        ...
        <Button
          ...
          className={`h-8 w-8 ml-auto ${isFullscreen ? "text-[var(--admin-primary)] bg-[var(--admin-primary)]/10" : "text-[var(--admin-text)]"}`}
        >
      ...
      {showLink && (
        <div className="absolute z-20 top-12 left-0 flex items-center gap-2 p-2 rounded-lg border border-[var(--admin-border)] bg-[var(--admin-card)] shadow-lg">
          ...
          />
          <Button size="sm" onClick={addLink} className="admin-btn-primary h-8 text-xs">
            Apply
          </Button>
        </div>
      )}
      ...
      {showYouTube && (
        <div className="absolute z-20 top-12 left-0 w-80 p-4 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card)] shadow-xl">
          ...
          <div className="flex justify-end gap-2 mt-3">
            <Button size="sm" variant="ghost" onClick={() => { setShowYouTube(false); setYoutubeUrl(""); }} className="text-[var(--admin-muted)] text-xs">
              Cancel
            </Button>
            <Button size="sm" onClick={addYouTube} disabled={!youtubeUrl.trim()} className="admin-btn-primary text-xs">
      ...
      {/* Main Editor Body */}
      <div className={isFullscreen ? "max-w-3xl mx-auto w-full flex-1" : ""}>
        <EditorContent editor={editor} />
      </div>

      {/* Live Content Statistics Bar */}
      <div className="flex items-center justify-between px-3 py-1.5 mt-1 border-t border-[var(--admin-border)]/50 text-[11px] text-[var(--admin-muted)] bg-[var(--admin-hover-bg)]/20 rounded-b">
        ...
        <div className="flex items-center gap-1 text-[var(--admin-muted)] font-medium">
      ...
      {showSlash && filteredCommands.length > 0 && (
        <div
          ref={slashMenuRef}
          className="absolute z-30 mt-1 w-60 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-card)] shadow-xl overflow-hidden"
        >
          {filteredCommands.map((cmd, idx) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-left text-[var(--admin-text)] hover:bg-[var(--admin-hover-bg)] ${idx === slashIndex ? "bg-[var(--admin-hover-bg)]" : ""}`}
            >
              {cmd.icon}
              {cmd.label}
            </button>
...
function ToolbarButton({
  ...
  command: string | (() => void);
  icon: React.ReactNode;
  active: string | Record<string, string | Record<string, number>>;
  title: string;
}) {
