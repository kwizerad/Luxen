...
export async function getAdminStats(): Promise<ActionResult<AdminStats>> {
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayUsers = (weekUsers || []).filter((u: { created_at: string; role?: string }) => {
        const d = new Date(u.created_at);
        return d >= day && d < dayEnd;
      });

      const userCount = dayUsers.length;
      const studentCount = dayUsers.filter((u: { role?: string }) => !u.role || u.role.toLowerCase() === "student").length;
