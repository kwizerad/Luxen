...
export function PermissionsMatrix({ permissions, onChange, disabled }: PermissionsMatrixProps) {
  ...
  return (
    <div className="space-y-1">
      {/* Header */}
      <div className="grid grid-cols-[1fr_3.5rem_3.5rem_3.5rem] gap-2 sm:gap-4 px-3 py-2 text-xs font-medium text-[var(--admin-muted)] border-b border-[var(--admin-border)]">
        <span>{t("feature")}</span>
        <span className="text-center">{t("permNone")}</span>
        <span className="text-center">{t("permReadOnly")}</span>
        <span className="text-center">{t("permReadWrite")}</span>
      </div>
      ...
      {PERMISSION_SECTIONS.map(({ key, labelKey }) => {
        ...
        return (
          <div
            key={key}
            className="grid grid-cols-[1fr_3.5rem_3.5rem_3.5rem] gap-2 sm:gap-4 px-3 py-2.5 items-center rounded-lg hover:bg-[var(--admin-hover-bg)] transition-colors"
          >
            <span className="text-sm font-medium text-[var(--admin-text)]">
              {t(labelKey) || key}
            </span>
            {(["none", "read_only", "read_write"] as PermissionAccess[]).map((access) => (
              <div key={access} className="flex items-center justify-center">
                <button
                  type="button"
                  disabled={disabled}
