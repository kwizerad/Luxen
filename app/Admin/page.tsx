...
export default function AdminDashboard() {
  ...
  const [weeklyRegistrations, setWeeklyRegistrations] = useState<AdminStats["weeklyRegistrations"]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (data.weeklyRegistrations) setWeeklyRegistrations(data.weeklyRegistrations);
  ...
  // Student registration chart data (past 7 days)
  const registrationChartData = weeklyRegistrations && weeklyRegistrations.length > 0
    ? weeklyRegistrations.map((d) => ({
        day: d.day,
        date: d.date,
        fullDate: d.fullDate,
        students: d.students,
  ...
  const totalNewStudentsThisWeek = stats?.newStudentsThisWeek ?? registrationChartData.reduce((sum, item) => sum + (item.students || 0), 0);
  const maxDayRegistration = registrationChartData.reduce((max, item) => item.students > max.students ? item : max, { day: "-", students: 0, fullDate: "" });
  const avgStudentsPerDay = (totalNewStudentsThisWeek / 7).toFixed(1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat, i) => {
          ...
          return (
            ...
            >
              <Link href={stat.href} className="admin-card stat-card p-5 block group relative overflow-hidden">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500">
                ...
                <div className="text-xs text-[var(--admin-muted)] mt-1 font-medium truncate">
                  {stat.title}
                </div>
              </Link>
            </motion.div>
      ...
      {/* Student Registrations (Bar Chart) + Platform Distribution (Donut) */}
      <div className="grid lg:grid-cols-3 gap-5">
        {/* New Student Registrations Bar Chart */}
        <motion.div
          className="admin-card p-6 lg:col-span-2 flex flex-col justify-between"
          custom={6}
          variants={fadeUp}
          initial="hidden"
          animate="visible"
        >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#10B981]/15 text-[#10B981]">
                    <UserPlus className="w-4 h-4" />
                  </div>
                  <div>
                    <h3 className="admin-card-title">New Student Registrations</h3>
                    <p className="text-xs text-[var(--admin-muted)] mt-0.5">Daily new student registrations over the past week</p>
                  </div>
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#10B981]/10 border border-[#10B981]/20 text-[#10B981] text-xs font-semibold">
                  <span>+{totalNewStudentsThisWeek}</span>
                  <span className="text-[var(--admin-muted)] font-normal">this week</span>
                </div>
                {maxDayRegistration.students > 0 && (
                  <div className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-full bg-[var(--admin-input-bg)] border border-[var(--admin-border)] text-xs text-[var(--admin-muted)]">
                    <span>Peak:</span>
                    <span className="font-semibold text-[var(--admin-text)]">{maxDayRegistration.day} ({maxDayRegistration.students})</span>
                  </div>
                )}
                <div className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 rounded-full bg-[var(--admin-input-bg)] border border-[var(--admin-border)] text-xs text-[var(--admin-muted)]">
                  <span>Avg:</span>
                  <span className="font-semibold text-[var(--admin-text)]">{avgStudentsPerDay}/day</span>
                </div>
              </div>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: "var(--admin-muted)", fontWeight: 500, marginBottom: "4px" }}
                    formatter={(value: any) => [`${value} student${Number(value) === 1 ? "" : "s"}`, "New Registrations"]}
                    labelFormatter={(label: any, payload: any) => {
                      const item = payload?.[0]?.payload;
                      return item?.fullDate ? `${item.fullDate}` : `${label ?? ""} (Past Week)`;
                    }}
                  />
                  <Bar
                    dataKey="students"
          ...
          {/* Daily breakdown pills below chart */}
          <div className="grid grid-cols-7 gap-1 sm:gap-2 mt-4 pt-3 border-t border-[var(--admin-border)]">
            {registrationChartData.map((d) => {
              const isPeak = maxDayRegistration.students > 0 && d.students === maxDayRegistration.students;
              return (
                <div
                  key={d.day}
                  className={`text-center py-1.5 px-1 rounded-lg transition-colors ${
                    isPeak
                      ...
                      : "bg-[var(--admin-input-bg)] border border-transparent"
                  }`}
                >
                  <p className="text-[10px] sm:text-xs text-[var(--admin-muted)] font-medium truncate">{d.day}</p>
                  <p className={`text-xs sm:text-sm font-bold mt-0.5 ${d.students > 0 ? "text-[#4ADE80]" : "text-[var(--admin-muted)]"}`}>
                    {d.students}
                  </p>
                </div>
              );
      ...
      {/* Visitor Geographic Telemetry Map */}
      <motion.div
        custom={7.5}
        variants={fadeUp}
        initial="hidden"
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              {!recentActivity?.categories?.length && !recentActivity?.questions?.length && !recentActivity?.users?.length && (
                <div className="text-center py-8 text-sm text-[var(--admin-muted)] col-span-3">No recent activity</div>
              )}
            </div>
          )}
        </div>
