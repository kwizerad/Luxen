...
import { CheckCircle, XCircle, Trophy, ArrowRight, Home, AlertCircle, AlertTriangle, BookOpen, Shield, ShieldAlert, HelpCircle, FileText, Play, LogOut, Monitor, Clock, Hash, ArrowLeft, History, ChevronLeft, ChevronRight, Check } from "lucide-react";
...
export default function TakeExamPage() {
  ...
  useEffect(() => {
    ...
    const checkExamHistory = async () => {
      ...
      const [res1, res2] = await Promise.allSettled([
        ...
        supabase.from("module_exam_attempts").select("*", { count: "exact", head: true }).eq("user_id", user.id),
      ]);
      const count1 = res1.status === "fulfilled" ? (res1.value.count ?? 0) : 0;
      const count2 = res2.status === "fulfilled" ? (res2.value.count ?? 0) : 0;
      setHasExamHistory(count1 + count2 > 0);
    };
  ...
  useEffect(() => {
    ...
    const handleAbortExamNavigation = () => {
      if (exam && !showResults && !isSubmittingOnExitRef.current) {
        submissionReasonRef.current = 'page_closed';
        isSubmittingOnExitRef.current = true;
        setIsSubmittingOnExit(true);
    ...
    window.addEventListener('abort-exam-navigation', handleAbortExamNavigation);
    ...
    return () => {
      ...
      window.removeEventListener('abort-exam-navigation', handleAbortExamNavigation);
  ...
  // Navigate questions with Left/Right arrow keys & select options with keyboard on desktop
  useEffect(() => {
    if (typeof window === 'undefined') return;
  ...
  }, [exam, showResults, showExamInstructions, activeQuestion]);
  ...
  const onTouchEnd = useCallback(() => {
    ...
    setTouchEndX(null);
    setTouchEndY(null);
  }, [touchStartX, touchEndX, touchStartY, touchEndY, exam, currentIndex, showExamInstructions]);
  ...
  const unansweredCount = useMemo(() => {
    ...
    return Math.max(0, exam.questions.length - answeredCount);
  }, [exam?.questions, answeredCount]);

  const visibleQuestionWindow = useMemo(() => {
    if (!exam?.questions) return [];
    const total = exam.questions.length;
    if (total <= 7) {
      return exam.questions.map((q, i) => ({ question: q, index: i }));
    }
    let start = currentIndex - 3;
    if (start < 0) start = 0;
    if (start + 7 > total) start = total - 7;
    return exam.questions.slice(start, start + 7).map((q, i) => ({
      question: q,
      index: start + i,
    }));
  }, [exam?.questions, currentIndex]);

  const handleInitiateManualSubmit = () => {
    if (!exam) return;
    const count = Object.keys(userAnswers).length;
    if (count === 0) {
      setAlertTitle(t("noAnswersSelected") || "No answers selected");
      setAlertMessage(t("noAnswersSelectedMessage") || "Please select at least one answer before submitting the exam.");
      setAlertType("warning");
      setShowAlert(true);
      return;
    }
  ...
  return (
    <div className={`bg-transparent ${isExamActive ? 'select-none' : ''}`}>
              ...
              <div className="flex items-center justify-between gap-3 sm:gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between text-[10px] sm:text-[11px] mb-0.5">
                    ...
                    <span className="text-muted-foreground tabular-nums font-medium">{answeredCount} / {exam.questions.length}</span>
                  </div>
                  <div className="relative h-1.5 sm:h-2 rounded-full overflow-hidden border border-white/10 dark:border-white/5 bg-white/30 dark:bg-white/5 backdrop-blur-md shadow-[inset_0_1px_2px_rgba(255,255,255,0.4),inset_0_-1px_2px_rgba(15,23,42,0.08)] dark:shadow-[inset_0_1px_2px_rgba(255,255,255,0.08),inset_0_-1px_2px_rgba(0,0,0,0.4)]">
                    <div
                ...
                <div className="text-right shrink-0">
                  <div className="text-[9px] sm:text-[10px] text-muted-foreground leading-none mb-0.5">{t("timeLeft")}</div>
                  <div className={`text-base sm:text-lg font-bold tabular-nums leading-tight ${secondsLeft !== null && secondsLeft < 60 ? "text-red-600 animate-pulse" : ""}`}>
                    {formatTime(secondsLeft)}
                  </div>
                </div>
              </div>

              {/* Horizontal Question Numbers Strip (Compact slim buttons) */}
              <div className="pt-1 border-t border-border/30 flex items-center justify-between gap-1">
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
                  ...
                  disabled={currentIndex === 0 && !showExamInstructions}
                  className="h-6 w-6 sm:h-7 sm:w-7 rounded-full shrink-0 text-muted-foreground hover:text-foreground hover:bg-muted/60 disabled:opacity-25"
                  aria-label="Previous question"
                >
                  <ChevronLeft className="h-3.5 w-3.5" />
                </Button>

                <div className="flex-1 flex items-center justify-center gap-1 sm:gap-1.5 overflow-hidden py-0.25">
                  <AnimatePresence mode="popLayout" initial={false}>
                    {visibleQuestionWindow.map(({ question: q, index: i }) => {
                      const isCurrent = !showExamInstructions && i === currentIndex;
                      ...
                      return (
                        <motion.button
                          ...
                          className={cn(
                            "relative flex flex-col items-center justify-center h-6 w-6 sm:h-7 sm:w-7 rounded-md sm:rounded-lg text-[10px] sm:text-xs font-bold transition-all select-none shrink-0",
                            isCurrent
                              ? "bg-primary text-primary-foreground shadow-sm ring-1.5 ring-primary ring-offset-1 ring-offset-background scale-105 z-10"
                              : isAnswered
                ...
                <Button
                  ...
                  disabled={!showExamInstructions && currentIndex >= exam.questions.length - 1}
                  className="h-6 w-6 sm:h-7 sm:w-7 rounded-full shrink-0 text-muted-foreground hover:text-foreground hover:bg-muted/60 disabled:opacity-25"
        ...
        <>
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                size="sm"
                onClick={() => handleInitiateManualSubmit()}
          ...
          {showExamInstructions ? (
            <ExamInstructionsView
              ...
              onStartQuestions={() => setShowExamInstructions(false)}
              examTitle={categories.find((c) => c.id === categoryId)?.name || "Exam"}
            />
          ) : (
            <Card
            ...
            >
              <CardHeader className="p-3 sm:p-5 pb-2 sm:pb-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="text-xs text-muted-foreground font-medium">
                    {answeredCount} / {exam.questions.length} {t("answered") || "Answered"}
                  </div>
                </div>
                <CardDescription className="text-[11px] sm:text-xs">
                  {t("examDurationLabel")}: {exam.settings.duration_minutes}m · {t("examQuestionsLabel")}: {exam.questions.length}
                  <span className="text-xs text-muted-foreground ml-2">({t("swipeToNavigate")})</span>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 sm:space-y-4 p-3 pt-0 sm:p-6 sm:pt-0">
                {activeQuestion ? (
                  ...
                  <div className="flex items-center justify-between pt-1.5 sm:pt-2 border-t border-border/40 mt-2">
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                    ...
                    {currentIndex === exam.questions.length - 1 ? (
                      <Button
                        ...
                        onClick={() => handleInitiateManualSubmit()}
                        disabled={submittingExam || answeredCount === 0}
                        className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold min-w-[110px] shadow-sm"
                      >
                        {submittingExam ? (
                          t("submitting")
                        ) : (
      ...
      <Dialog open={showInstructions} onOpenChange={setShowInstructions}>
        <DialogContent
          className="max-w-2xl sm:max-w-3xl max-h-[85vh] sm:max-h-[90vh] overflow-y-auto"
        ...
        >
          ...
          <div className="space-y-3 sm:space-y-4 py-2 sm:py-4">
            ...
            {/* Acceptance - High Contrast, Large Tap Area */}
            <div
              onClick={() => setInstructionsAccepted(!instructionsAccepted)}
              className={cn(
                "flex items-center gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border-2 transition-all cursor-pointer select-none",
                instructionsAccepted
                  ? "bg-primary/10 border-primary shadow-sm dark:bg-primary/20"
                  : "bg-muted/80 hover:bg-muted border-primary/50 dark:border-primary/60"
              )}
            >
              <div className="flex items-center justify-center shrink-0">
                <Checkbox
                  ...
                  className="h-6 w-6 sm:h-7 sm:w-7 rounded-lg border-2 border-primary bg-background dark:bg-card"
                />
              </div>
              <div className="flex-1 min-w-0">
                <label
                  htmlFor="accept"
                  className="text-xs sm:text-sm font-bold text-foreground leading-snug cursor-pointer block"
                >
                  {t("acceptExamInstructions") || "I have read and agree to all exam instructions and security rules."}
                </label>
      ...
      <Dialog open={showPreSubmitModal} onOpenChange={setShowPreSubmitModal}>
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-xl max-h-[calc(100dvh-4rem)] overflow-hidden flex flex-col p-0 rounded-2xl border-primary/20 shadow-2xl">
          <DialogHeader className="p-4 sm:p-6 pb-3 border-b bg-card">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-xl bg-primary/15 text-primary border border-primary/30 shrink-0">
              ...
              <div>
                ...
                <DialogDescription className="text-xs sm:text-sm text-muted-foreground mt-0.5">
                  {t("reviewQuestionsBeforeSubmit") || "Review your answered and unanswered questions before final submission."}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
              <div className="p-3 sm:p-3.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span className="text-xs sm:text-sm font-semibold">{t("answered") || "Answered"}</span>
                </div>
                <span className="text-base sm:text-xl font-extrabold tabular-nums">
                  {answeredCount} / {exam?.questions.length ?? 0}
                </span>
              </div>

              <div className={cn(
                "p-3 sm:p-3.5 rounded-xl border flex items-center justify-between transition-colors",
                unansweredCount > 0
                  ? "border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300"
                  : "border-border bg-muted/40 text-muted-foreground"
              )}>
                <div className="flex items-center gap-2">
                  <AlertCircle className={cn("h-4 w-4 sm:h-5 sm:w-5 shrink-0", unansweredCount > 0 ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground")} />
                  <span className="text-xs sm:text-sm font-semibold">{t("notAnswered") || "Not Answered"}</span>
                </div>
                <span className="text-base sm:text-xl font-extrabold tabular-nums">
                  {unansweredCount}
                </span>
            ...
            {/* Warning note if unanswered questions exist */}
            {unansweredCount > 0 && (
            ...
            {/* Questions Grid with Answered / Not Answered Status */}
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                {t("questionsOverview") || "Questions Overview"}
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 max-h-[260px] overflow-y-auto pr-1">
                {exam?.questions.map((q, i) => {
                  const isAnswered = !!userAnswers[q.id]?.selectedAnswer;
                  const selectedOpt = userAnswers[q.id]?.selectedAnswer;
                  return (
                    <button
                      key={`review-q-${q.id}`}
                      type="button"
                      ...
                      className={cn(
                        "flex items-center justify-between p-2.5 rounded-xl border text-left transition-all hover:scale-[1.02] active:scale-[0.98]",
                        isAnswered
                          ? "border-emerald-500/30 bg-emerald-500/5 hover:bg-emerald-500/10 text-foreground"
                          : "border-amber-500/30 bg-amber-500/5 hover:bg-amber-500/10 text-amber-900 dark:text-amber-200"
                      )}
                      title={`Go to question ${i + 1}`}
                    >
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className="text-xs font-bold shrink-0">Q{i + 1}</span>
                        {isAnswered ? (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-emerald-500/15 text-emerald-700 dark:text-emerald-300">
                            {selectedOpt ? `${selectedOpt}` : (t("answered") || "Answered")}
                          </span>
                        ) : (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-700 dark:text-amber-300 truncate">
                            {t("notAnswered") || "Not Answered"}
                          </span>
                        )}
                      </div>
                      {isAnswered ? (
                        <CheckCircle className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400 shrink-0 ml-1" />
                      ) : (
                        <AlertCircle className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400 shrink-0 ml-1" />
                      )}
                    </button>
                  );
                })}
          ...
          <DialogFooter className="p-4 sm:p-5 border-t bg-muted/20 flex-col-reverse sm:flex-row gap-2 sm:gap-3">
            <Button
              type="button"
              variant="outline"
            ...
            >
              {t("reviewQuestions") || "Review Questions"}
            </Button>
            <Button
              type="button"
              onClick={() => {
                setShowPreSubmitModal(false);
                handleSubmitExamRef.current?.(false);
              }}
              disabled={submittingExam}
              className="w-full sm:w-auto font-bold min-w-[140px]"
            >
              {submittingExam ? (
                t("submitting")
              ) : (
                <>
                  {t("confirmSubmit") || t("submit") || "Confirm & Submit"}
