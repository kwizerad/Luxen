...
export function dbLessonToUI(l: CourseLesson): Lesson {
  ...
  if (Array.isArray(rawTopics)) {
    ...
    topics = rawTopics.map((t: any) =>
      typeof t === "string"
        ? { id: crypto.randomUUID(), title: t, content: tiptapEmptyDoc(), estimated_minutes: 0, audioUrl: undefined }
        : {
            id: t.id || crypto.randomUUID(),
            title: t.title || "",
            content: t.content || tiptapEmptyDoc(),
            estimated_minutes: t.estimated_minutes || 0,
            audioUrl: t.audioUrl || t.audio_url || undefined,
          }
    );
  }
  return {
