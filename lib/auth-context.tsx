...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshUser = async () => {
    try {
      ...
      setSession((sessionResult?.data?.session || null) as Session | null);
    } catch (error: any) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (
      ...
      let sessionUser: any = session?.user ? {
        ...
        avatar_url:
          ...
          session.user.user_metadata?.google_avatar_url ||
      ...
      if (session?.user?.id) {
        try {
          ...
          if (profile) {
            ...
            if (isAdminUser && resolvedAvatar && (
              resolvedAvatar.includes("irembo.gov.rw") ||
              resolvedAvatar.includes("nida.gov.rw") ||
              resolvedAvatar.includes("citizen-photos")
            )) {
              resolvedAvatar =
                session.user.user_metadata?.google_avatar_url ||
                session.user.user_metadata?.picture ||
            ...
            sessionUser = {
              ...
              user_metadata: {
                ...
                ...(resolvedAvatar ? { avatar_url: resolvedAvatar } : {}),
