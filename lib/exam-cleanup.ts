...
export async function cleanupStaleExamAttempts(options: {
  ...
  try {
    ...
    // 3. Clean up only stale, unstarted pending or cancelled exam_challenges older than 24 hours
    // IMPORTANT: NEVER delete 'completed' or active challenges so group exam history is permanently preserved
    try {
