...
import { Search, Send, Loader2, Users, UserPlus, MessageCircle, Trophy, X, Check, CheckCheck, ArrowLeft, Eye, EyeOff, Bell, Clock, Play, Smile, Heart, Sparkles } from "lucide-react";
  ...
  const filteredChallenges = challenges
    ...
    .filter((challenge) => {
      ...
      if (activeTab === "ongoing") {
        return challenge.status === "pending" || challenge.status === "active";
      }
      if (activeTab === "completed") {
        return challenge.status === "completed" || userParticipation?.status === "completed";
      }
      return false;
    })
    .filter((challenge) => {
        ...
        {filteredChallenges.length === 0 ? (
          <div className="text-center py-12 px-4 rounded-2xl border border-dashed bg-card/40 my-4">
            ...
            <p className="text-xs text-muted-foreground max-w-xs mx-auto mb-4">
              {activeTab === "ongoing"
                ? "Ibizamini byose wemeye kwinjiramo kandi bikirimo gukorwa biboneka hano."
                : "Ibizamini by'amatsinda mwese mwarangije hamwe n'amanota yabyo bibikwa hano."}
            </p>
            {onOpenCreateExam && (
              <Button
                size="sm"
              ...
              >
                <Trophy className="h-3.5 w-3.5" />
                <span>Tegura Ikizamini Cy'Itsinda</span>
              </Button>
            )}
          </div>
        ) : (
          filteredChallenges.map((challenge) => {
            ...
            const isOngoing = (userParticipation?.status === "joined" || challenge.creator_id === user?.id) && challenge.status === "active";
            ...
            return (
              ...
              >
                  ...
                  <div className="space-y-1.5 mb-3">
                    {challenge.participants
                      ...
                      .map((participant, index) => (
                        <div
                          key={participant.id}
                          className="flex items-center justify-between text-xs px-2.5 py-1 rounded-lg bg-muted/40 border border-border/30"
                        >
                          ...
                          <span className="font-bold text-foreground text-[11px]">
                            {participant.exam_attempt?.percentage || 0}%
                          </span>
                        </div>
                      ))}
                  </div>
                ...
                <div>
                  ...
                  {/* Case 6: Completed Challenge - View Rankings */}
                  {isCompleted && (
                    <Button
                      size="sm"
                      variant="outline"
                    ...
                    >
                      ...
                      <span>{t("viewRankings") || "Reba Uko Bakurikirana mu Manota"}</span>
                    </Button>
                  )}
                </div>
              </div>
            ...
            {pendingReceivedRequests.length > 0 ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-md mx-auto text-center">
                ...
                <h3 className="text-lg font-bold text-foreground mb-1.5">
                  {t("examInvitations") || "Ubutumire mu Bizamini"}
                </h3>
                <p className="text-xs text-muted-foreground mb-6 leading-relaxed">
                  Tegura ikizamini cy'itsinda maze utumire abanyeshuri bagenzi bawe muhatane mu buryo bw'ako kanya (real-time).
                </p>
                ...
                <div className="grid grid-cols-2 gap-3 w-full mb-6 text-left">
                  <div className="p-3 rounded-xl border bg-card/60">
                    <div className="flex items-center gap-1.5 font-bold text-xs text-foreground mb-1">
                      ...
                      <span>Amasegonda 30</span>
                    </div>
                    <p className="text-[11px] text-muted-foreground leading-tight">
                      Ubutumire bumara amasegonda 30 gusa yo kubwemera.
                    </p>
                  </div>
                  ...
                  <div className="p-3 rounded-xl border bg-card/60">
                    <div className="flex items-center gap-1.5 font-bold text-xs text-foreground mb-1">
                      ...
                      <span>Amanota ako kanya</span>
                    </div>
                    <p className="text-[11px] text-muted-foreground leading-tight">
                      Reba uko mukurikirana mu manota mugitangira no mukirangiza.
                    </p>
                  </div>
                </div>
                ...
                >
                  <Trophy className="h-4 w-4" />
                  <span>Tegura Ikizamini Gishya</span>
                </Button>
              </div>
            ) : (
              <>
