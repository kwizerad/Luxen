...
import { canViewUserAvatar, getDisplayAvatarUrl } from "@/lib/avatar-helper";
...
function ExamInvitationsContent({ 
  ...
  useEffect(() => {
    ...
    const channelName = `exam_challenges:${user.id}`;
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 mb-3 bg-muted/30 dark:bg-muted/20 rounded-xl p-2.5 border border-border/40 text-xs">
                  ...
                  {/* Timestamps: Invited At & Ended/Expired At */}
                  <div className="flex flex-wrap items-center justify-between gap-1.5 pt-1.5 border-t border-border/30 text-[11px] text-muted-foreground">
                    <div className="flex items-center gap-1">
                      <Clock className="h-3 w-3 text-primary" />
                      <span>{t("invitedAt") || "Invited"}: {new Date(challenge.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    {isCompleted && (
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-border/30">
                    ...
                    <div className="flex items-center -space-x-1.5">
                      {challenge.participants?.slice(0, 4).map((participant) => {
                        const canShow = canViewUserAvatar(
                          participant.profile?.avatar_url,
                          participant.user_id || participant.profile?.id,
                          user?.id,
                          user?.user_metadata?.role
                        );
                        return (
                          <Avatar key={participant.id} className="w-5 h-5 border-2 border-card ring-1 ring-border/20">
                            {canShow && participant.profile?.avatar_url ? (
...
export function ClassmatesView({ navigate }: ClassmatesViewProps) {
  ...
  const selectedFriendRef = useRef<FriendProfile | null>(selectedFriend);
  ...
  // Fetch challenges for chat with a specific user
  ...
  // Realtime subscription for exam challenge updates (invites + active challenges in chat)
  useEffect(() => {
    if (!user?.id) return;
    const channelName = `exam_invite_count:${user.id}`;
    const channel = supabase
      .channel(channelName)
      .on(
        "postgres_changes",
        ...
        () => {
          ...
          if (selectedFriendRef.current) {
            fetchChallenges(selectedFriendRef.current.id);
          }
        }
      )
      .on(
  ...
  }, [user?.id, supabase, fetchPendingExamInvites, fetchChallenges]);

  // Global realtime subscription for new message notifications
  useEffect(() => {
    if (!user?.id || typeof window === "undefined") return;

    const channelName = `chat_notifications:${user.id}`;
    const channel = supabase
      .channel(channelName)
      .on(
        "postgres_changes",
        ...
        async (payload: any) => {
          ...
          if (conversationIdRef.current === newMsg.conversation_id) return;
          ...
          const senderName = senderProfile?.full_name || senderProfile?.username || "Friend";
  ...
  useEffect(() => {
    ...
    const reqChannelName = `classmate_requests_rt:${user.id}`;
    ...
    // Subscribe to user_profiles changes for online status and profile updates
    const profileChannelName = `user_profiles_rt:${user.id}`;
          ...
          setRequests((prev) =>
            prev.map((r) =>
              r.other_user.id === updated.id
                ? { ...r, other_user: { ...r.other_user, ...updated } }
                : r
            )
          );
        }
  ...
  useEffect(() => {
    ...
    const channelName = `chat_messages:${conversationId}`;
    const channel = supabase
      .channel(channelName)
      .on(
        "postgres_changes",
        ...
        (payload: any) => {
          ...
          if (selectedFriendRef.current) {
            setFriendLastMessages((prev) => {
              return new Map(prev).set(selectedFriendRef.current!.id, {
                message: newMsg.message,
                time: newMsg.created_at,
                unread: 0,
              });
  ...
  const getAvatarUrl = (profile?: FriendProfile | null) => {
    ...
    return getDisplayAvatarUrl(profile.avatar_url, profile.id, user?.id, user?.user_metadata?.role);
  };
      ...
      <Dialog open={showInviteModal} onOpenChange={setShowInviteModal}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="gap-2 sm:gap-0 pt-2 border-t">
            <Button variant="outline" onClick={() => setShowInviteModal(false)} className="rounded-xl h-10 text-xs sm:text-sm">
              {t("cancel") || "Cancel"}
            </Button>
            <Button
              onClick={handleCreateChallenge}
              disabled={creatingChallenge || !selectedCategory || selectedInvitees.size === 0}
              className="rounded-xl h-10 text-xs sm:text-sm font-bold bg-amber-600 hover:bg-amber-700 text-white shadow-sm gap-2"
            >
              {creatingChallenge ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
              ...
              <span>
                {selectedInvitees.size > 0
                  ? `${t("createChallenge") || "Create Challenge"} (${selectedInvitees.size})`
