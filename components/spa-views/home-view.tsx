...
export function HomeView({ navigate }: HomeViewProps) {
  ...
  const activeGroupChallenges = useMemo(() => {
    ...
    return groupChallenges.filter((c) => {
      ...
      const isInvitedOrJoined = myParticipation && (myParticipation.status === "pending" || myParticipation.status === "joined" || myParticipation.status === "ready");
      return isCreator || isInvitedOrJoined;
    });
                    ...
                    <div className="min-w-0 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge className="bg-amber-500 text-amber-950 font-bold text-[10px] px-2 py-0.5 shadow-xs">
                          {isPendingInvite ? (t("groupExamInvitation") || "Ubutumire bw'Ikizamini") : (t("activeGroupExam") || "Ikizamini cy'Itsinda Kirafunguye")}
                        </Badge>
                        {isPendingInvite && (
