...
export function AdminDockNav({ user, isPrimaryAdmin }: AdminDockNavProps) {
  ...
  const canViewStudentsTab = canAccess(user, "students") || canAccess(user, "users");
  const canViewExamsTab = canAccess(user, "exams");
  const canViewSettingsTab = true; // All admins have access to at least Account settings
