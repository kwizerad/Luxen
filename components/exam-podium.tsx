...
export function ExamPodium({ participants }: ExamPodiumProps) {
  ...
  const topPassed = top3.length > 0 && top3[0].scorePercentage >= 50;
  ...
  const rank1Config = {
    ...
    label: topPassed ? (t("champion") || "Champion") : (t("firstPlace") || "1st Place"),
    delay: 0.1,
          ...
          return (
            <div key={idx} className="flex flex-col items-center flex-1 max-w-[120px]">
              ...
              >
                ...
                >
                  {canViewUserAvatar(p.profile?.avatar_url, p.user_id || p.userId || p.profile?.id, user?.id, user?.user_metadata?.role) && p.profile?.avatar_url && (
