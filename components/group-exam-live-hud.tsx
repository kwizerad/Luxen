...
import { Users, CheckCircle2, XCircle, Flame, X, User, Clock } from "lucide-react";
...
export function GroupExamLiveHUD({
  ...
  return (
    <>
      {/* Floating Action Button (Fixed on screen, mobile top-left & desktop bottom-right accessible) */}
      <div className="fixed top-20 left-3 sm:top-auto sm:bottom-6 sm:left-auto sm:right-6 z-40 select-none">
        <motion.button
          type="button"
          ...
          whileTap={{ scale: 0.95 }}
          className={cn(
            "relative flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-3 rounded-full shadow-lg transition-all duration-200 border",
            isOpen
          ...
          <>
            ...
            <motion.div
              ...
              className={cn(
                ...
                // Positioning: top-left dropdown on mobile (away from bottom next button), floating corner card on desktop
                "top-32 left-3 right-3 sm:top-auto sm:bottom-20 sm:left-auto sm:right-6 sm:w-96 max-h-[70vh]"
              )}
            >
              {/* Header */}
              ...
              <div className="p-3 overflow-y-auto space-y-2 max-h-[55vh]">
                {participants.map((p, index) => {
                  ...
                  const isInProgress = !isCompleted && !isAbandoned && !isPending && !isReadyOrJoined;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <div className="relative shrink-0">
                          <Avatar className="h-8 w-8 border">
                            {canViewUserAvatar(p.profile?.avatar_url, p.user_id, currentUserId) && p.profile?.avatar_url && (
                          ...
                          {isCompleted ? (
                            ...
                            <span className="absolute -bottom-0.5 -right-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-muted text-muted-foreground ring-1 ring-background">
                              <Clock className="h-2.5 w-2.5" />
                            </span>
                          ) : isReadyOrJoined ? (
                            <span className="absolute -bottom-0.5 -right-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-sky-500 text-white ring-1 ring-background">
                          ...
                          <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground mt-0.5">
                            {isCompleted ? (
                              ...
                              <span className="text-muted-foreground font-medium flex items-center gap-1">
                                ...
                                <span>{t("notJoinedYet") || "Has not joined yet"}</span>
                              </span>
                            ) : isReadyOrJoined ? (
                              <span className="text-sky-600 dark:text-sky-400 font-medium flex items-center gap-1">
                                <Users className="h-3 w-3" />
                                <span>{p.status === "ready" ? (t("readyToStart") || "Ready to start") : (t("inLobbyWaiting") || "In lobby")}</span>
                              </span>
                            ) : (
                      ...
                      <div className="shrink-0 text-right flex flex-col items-end justify-center">
                        {isCompleted ? (
                          <div className="px-2 py-0.5 rounded-md font-bold text-xs tabular-nums border bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20">
                            100%
                          </div>
                        ) : isAbandoned ? (
                          <div className="px-2 py-0.5 rounded-md font-bold text-xs tabular-nums border bg-destructive/10 text-destructive border-destructive/20">
                            {t("left") || "Left"}
                          </div>
                        ) : isPending ? (
                          ...
                          <Badge variant="outline" className="text-[10px] text-sky-600 dark:text-sky-400 border-sky-500/30 bg-sky-500/10 font-semibold px-2 py-0.5">
                            {p.status === "ready" ? (t("ready") || "Ready") : (t("joined") || "In Lobby")}
                          </Badge>
                        ) : (
                          <div className="px-2 py-0.5 rounded-md font-bold text-xs tabular-nums border bg-primary/10 text-primary border-primary/20">
                            {progressPercent}%
                          </div>
                        )}
                      </div>
