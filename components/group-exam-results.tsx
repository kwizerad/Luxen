...
import { Trophy, Medal, Award, Clock, CheckCircle, XCircle, ArrowLeft, RotateCcw, Home, Crown, Flame, Users, Activity, TrendingUp, TrendingDown, Loader2 } from "lucide-react";
...
export function GroupExamResults({ challengeId, examResult, questions, onReset, onRetake }: GroupExamResultsProps) {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time participant, challenge and attempt updates
  ...
  const isWinnerPassed = winner ? (
    winner.score_percentage !== undefined && winner.score_percentage !== null
      ? Number(winner.score_percentage) >= 50 
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] p-4 pb-20">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Winner Hero Card (Only shown if winner passed the exam) */}
        {winner && isWinnerPassed && (
          <motion.div
              ...
              <div className="flex items-center gap-4 text-center sm:text-left flex-col sm:flex-row">
                <div className="relative">
                  ...
                  <Avatar className="h-16 w-16 sm:h-20 sm:w-20 ring-4 ring-amber-400/80 shadow-md">
                    {canViewUserAvatar(winner.avatar_url, winner.user_id, user?.id || examResult.user_id, user?.user_metadata?.role) && winner.avatar_url && (
                                ...
                                <span className="flex items-center gap-1.5 text-xs">
                                  {entry.status === "pending" ? (
                                    <>
                                      ...
                                      <span className="text-muted-foreground">{t("notJoinedYet") || "Has not joined yet"}</span>
                                    </>
                                  ) : entry.status === "joined" ? (
                                    <>
                                      <Users className="h-3 w-3 text-sky-500" />
                                      <span className="text-sky-600 dark:text-sky-400 font-medium">{t("inLobbyWaiting") || "In lobby"}</span>
                                    </>
                                  ) : entry.status === "ready" ? (
                                    <>
                                      <CheckCircle className="h-3 w-3 text-emerald-500" />
                                      <span className="text-emerald-600 dark:text-emerald-400 font-medium">{t("readyToStart") || "Ready to start"}</span>
                                    </>
                                  ) : (entry.status === "in_progress" || entry.status === "active") ? (
                                    <>
                                      <Loader2 className="h-3 w-3 text-amber-500 animate-spin" />
                                      <span className="text-amber-600 dark:text-amber-400 font-medium">{t("takingExamNow") || "Taking exam now..."}</span>
                                    </>
                                  ) : (entry.status === "declined" || entry.status === "rejected") ? (
                                    <>
                                      <XCircle className="h-3 w-3 text-rose-500" />
                                      <span className="text-rose-600 dark:text-rose-400">{t("declinedInvite") || "Declined invitation"}</span>
                                    </>
                                  ) : (entry.status === "left" || entry.status === "abandoned") ? (
                                    <>
                                      <XCircle className="h-3 w-3 text-zinc-500" />
                                      <span className="text-zinc-600 dark:text-zinc-400">{t("leftExam") || "Left exam"}</span>
                                    </>
                                  ) : (
                                    <>
                                      <Clock className="h-3 w-3 text-muted-foreground" />
                          ...
                          {entry.completed ? (
                            ...
                            <Badge
                              ...
                              className={cn(
                                ...
                                entry.status === "pending" && "text-muted-foreground bg-muted/40 border-border/60",
                                entry.status === "joined" && "text-sky-600 dark:text-sky-400 bg-sky-500/10 border-sky-500/30",
                                entry.status === "ready" && "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/30",
                                (entry.status === "in_progress" || entry.status === "active") && "text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/30",
                                (entry.status === "declined" || entry.status === "rejected") && "text-rose-600 dark:text-rose-400 bg-rose-500/10 border-rose-500/30",
                                (entry.status === "left" || entry.status === "abandoned") && "text-zinc-600 dark:text-zinc-400 bg-zinc-500/10 border-zinc-500/30"
                              )}
                            >
                              {entry.status === "pending" ? (t("pending") || "Pending") :
                               entry.status === "joined" ? (t("joined") || "In Lobby") :
                               entry.status === "ready" ? (t("ready") || "Ready") :
                               (entry.status === "in_progress" || entry.status === "active") ? (t("inProgress") || "In Progress") :
                               (entry.status === "declined" || entry.status === "rejected") ? (t("declined") || "Declined") :
                               (entry.status === "left" || entry.status === "abandoned") ? (t("left") || "Left") :
                               (t("pending") || "Pending")}
                            </Badge>
                          )}
                        </div>
