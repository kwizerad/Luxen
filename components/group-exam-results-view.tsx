...
export function GroupExamResultsView({ navigate, params }: GroupExamResultsViewProps) {
  ...
  const ProfileAvatar = ({ profile, size = "h-8 w-8" }: { profile?: any; size?: string }) => {
    ...
    const canShow = canViewUserAvatar(profile.avatar_url, targetId, user?.id, user?.user_metadata?.role);

    return (
      <Avatar className={size}>
        {canShow && profile.avatar_url && <AvatarImage src={profile.avatar_url} alt="" />}
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] pb-20 sm:pb-24">
      <div className="container mx-auto max-w-4xl px-4 py-5 sm:py-8">
        ...
        {/* Winner Completion Animation Banner (Only shown if winner passed the exam) */}
        {winner && isWinnerPassed && (
          <motion.div
              ...
              <div className="flex items-center gap-3.5 sm:gap-4 text-center sm:text-left flex-col sm:flex-row">
                <div className="relative">
                  ...
                  <Avatar className="h-16 w-16 sm:h-20 sm:w-20 ring-4 ring-amber-400/80 shadow-lg">
                    {canViewUserAvatar(winner.participant.profile?.avatar_url, winner.participant.user_id || winner.participant.profile?.id, user?.id, user?.user_metadata?.role) && winner.participant.profile?.avatar_url && (
        ...
        <div className="mb-6 rounded-xl border bg-card overflow-hidden shadow-xs">
          ...
          <motion.div layout className="divide-y divide-border">
            ...
            {incompleteParticipants.map((p) => {
              ...
              const isPending = statusStr === "pending" || statusStr === "invited";
              const isJoined = statusStr === "joined";
              const isReady = statusStr === "ready";
              const isInProgress = statusStr === "in_progress" || statusStr === "active";
              const isDeclined = statusStr === "declined" || statusStr === "rejected";
              const isLeft = statusStr === "left" || statusStr === "abandoned";
              const name = p.profile?.full_name || p.profile?.username || "Student";

              return (
                <motion.div
                  layout
                  key={p.id}
                  className="flex items-center justify-between gap-3 px-4 py-3 opacity-80 border-b last:border-b-0"
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground shrink-0">
                      {isPending ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {isPending
                          ? (t("notJoinedYet") || "Has not joined yet")
                          : isJoined
                          ? (t("inLobbyWaiting") || "In lobby")
                          : isReady
                          ? (t("readyToStart") || "Ready to start")
                          : isInProgress
                          ? (t("takingExamNow") || "Taking exam now...")
                          : isDeclined
                          ? (t("declinedInvite") || "Declined invitation")
                          : isLeft
                          ? (t("leftExam") || "Left exam")
                          : (t("pending") || "Pending")}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="shrink-0">
                    <Badge
                      ...
                      className={cn(
                        ...
                        isPending && "text-muted-foreground bg-muted/40 border-border/60",
                        isJoined && "text-sky-600 dark:text-sky-400 bg-sky-500/10 border-sky-500/30",
                        isReady && "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/30",
                        isInProgress && "text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/30",
                        isDeclined && "text-rose-600 dark:text-rose-400 bg-rose-500/10 border-rose-500/30",
                        isLeft && "text-zinc-600 dark:text-zinc-400 bg-zinc-500/10 border-zinc-500/30"
                      )}
                    >
                      {isPending
                        ? (t("pending") || "Pending")
                                  ...
                                  return (
                                    ...
                                    >
                                      <Avatar className="h-4 w-4 shrink-0">
                                        {canViewUserAvatar(c.profile?.avatar_url, c.user_id || c.profile?.id, user?.id, user?.user_metadata?.role) && c.profile?.avatar_url && (
